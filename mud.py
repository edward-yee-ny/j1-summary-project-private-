...
class Game:
    def __init__(self):
        ...
        def rungame(self, game, player, game_over):
            ...
            """)
            print(f"""
            STAGE 1: Preliminary Rounds /nComplete these rounds to advance further in the tournament""")
            # functions used
        ...
        save_data = extractJson("save_state.py")
        def load_save(game, save_data):
            for i in range(save_data):
                if save_data[i] == "right":
                    self.game.currentRoom
                elif save_data[i] == "left":
                elif saev_Data[i] == "forward":
game = Game()
